...
/* Modular distance */
const mod_dist = (x, y)=> {
    return min(mod((x - y),60),mod((y - x),60));
}
 ...
 */
const num_circle = (n, radius) => {
  if(n > 59) return null; /* Unless you are not on earth */
  
  push();
  
  /* Rotate to the current number */
  rotate(2 * n * Math.PI / 60);
  
  let i = -1;
  while(i++ < 59){ /* Print all numbers */
    
    /* Just for the styling */
    let distance = mod_dist(n,i);
    let mapped_d = map(distance,0,30,0,255);
...
function draw() {
  ...
  /* The 3 rings */
  num_circle(h, inner_radius + margin * 0); /* Yes, I am weired */
  num_circle(m, inner_radius + margin * 1); 
  num_circle(s, inner_radius + margin * 2); 
